  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    ...
    sparkSession.sql(
      """
        ...
        |                a.dn
        |         from education_online_dwd.dwd_member a
        |                  left join education_online_dwd.dwd_member_regtype b
        |                            on a.uid = b.uid
        |                                and a.dn = b.dn
        |                  left join education_online_dwd.dwd_base_ad c on a.ad_id = c.adid and a.dn = c.dn
        |                  left join
        |              education_online_dwd.dwd_base_website d on b.websiteid = d.siteid and b.dn = d.dn
        |                  left join education_online_dwd.dwd_pcentermempaymoney e
        |                            on a.uid = e.uid and a.dn = e.dn
        |                  left join education_online_dwd.dwd_vip_level f on e.vip_id = f.vip_id and e.dn = f.dn
        |         where a.dt = '${time}')
        |group by uid, dn, dt
